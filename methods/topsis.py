...
def prepare_info(res_dataset):
    ...
    max_info = f'Объект №{max_ind + 1} лучший. Количество очков = {res_dataset[max_ind]:.4f}'
    min_info = f'Объект №{min_ind + 1} худший. Количество очков = {res_dataset[min_ind]:.4f}'
    res = [f'Объект №{i + 1} = {x:.4f}' for i, x in enumerate(res_dataset)]
    res.append(max_info)
    res.append(min_info)
    res = '\n'.join(res)
    print('---------TOPSIS метод---------\n')
